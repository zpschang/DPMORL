...
def load_variables(load_path, variables=None, sess=None, **loaded_name):
    import joblib
    sess = sess or get_session()
    variables = variables or tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES)
    ...
    if isinstance(loaded_params, list):
        ...
        for v in variables:
            if "exp_pi" in v.name:
                name = v.name.replace('exp_pi', 'pi')
            else:
                name = v.name
            # print("Name: {} layer: {}".format(name, v))
            if name in loaded_params.keys():
                if len(loaded_name) == 0 or (len(loaded_name) and list(loaded_name.values())[0] in name):
                    # IPython.embed()
                    # import time
                    # time.sleep(10)
                    flag = 1
